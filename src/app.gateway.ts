...
export class AppGateway implements OnGatewayDisconnect{
  ...
  ALL = 'All'
  users = new Map() // socket.id => [userName, avatar]
  rooms = new Map([[this.ALL, []]]) // roomName => [... userIds]
  privateRooms = new Set()
  messages = new Map([[this.ALL, []]]) //  roomname => [...Message]
  activeUsers = new Set()


  @SubscribeMessage('test')
  test(client: Socket, payload: any): WsResponse {
    console.log('test')
    return { event: 'test', data: 'test' };
  }
  ...
  getRoomList(client: Socket): WsResponse {
